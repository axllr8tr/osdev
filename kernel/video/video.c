...
volatile u16  vga_textmode_buffer[VGA_HEIGHT * VGA_WIDTH]; // breaks shell!!
...
void _loc_scrollback(u16 ht) {
  if (ht < y) {
    memcpyw((u16p)&vga_textmode_mmio[0], (const u16p)&vga_textmode_mmio[ht * width], (height - ht) * width * 2);
    // memfillw((u16p)&vga_textmode_buffer[(height + ht) * width], 0xff00, (height - ht) * width);
    memfillw((u16p)&vga_textmode_mmio[coord_to_offs(0, height - ht)], (default_color << 8) | 0, width);
    y = height - ht;
  }
  else {
    y = 0;
